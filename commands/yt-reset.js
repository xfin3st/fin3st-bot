  ...
  async execute(interaction) {
    ...
    try {
      if (fs.existsSync(LAST_FILE)) {
        ...
        await interaction.reply('✅ YouTube-Cache wurde zurückgesetzt. Das nächste Video wird neu gepostet.');
      } else {
        await interaction.reply('ℹ️ Es gibt aktuell keine youtube_last.json.');
      }
    } catch (err) {
      console.error('❌ Fehler beim Löschen von youtube_last.json:', err);
      await interaction.reply('❌ Fehler beim Zurücksetzen der Datei.');
    }
  },
};
